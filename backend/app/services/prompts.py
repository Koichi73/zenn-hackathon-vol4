...
- **分割の基準**: 以下のようなタイミングを新しいステップとして切り分けてください。
   - 画面全体が遷移する（URLが変わる）
   - アプリケーションを移動する
   - 重要なモーダル/ダイアログが開閉する
- **OK例**: 次のシーンに進むタイミング
   - 初期設定中に「次へ」のボタンで画面遷移した
   - ブラウザ上で、GmailのタブからGoogleカレンダーのタブに移動した
   - エクセルからパワーポイントに切り替えた
- **NG例**: 1つのシーンにまとめるべき
   - 1つの画面内で「名前を入力」→「メールを入力」→「パスワードを入力」と操作する
      - 3つのシーンに分けるのではなく、「アカウント情報を設定する」などのタイトルで1つのシーンとして扱う
